        ...
        for idx, query_result in enumerate(self.query_results):
            if query_result.status != Result.StatusExtended.OK:
                ...
                if query_result.description.exception:
                    stack_trace = StackTraceReader.get_stack_trace(
                        stderr=query_result.description.exception
                    )
                    fatal_error = StackTraceReader.get_fatal_error(
                        stderr=query_result.description.exception
                    )
                    if stack_trace and fatal_error:
                        info += f"Exception:\n"
                        info += f"{fatal_error}\n\n"
                        info += f"Stack trace:\n"
                        info += f"{stack_trace}\n\n"
                        ...
                        info += f"Exception:\n"
                        info += f"{query_result.description.exception}\n"
                elif (
