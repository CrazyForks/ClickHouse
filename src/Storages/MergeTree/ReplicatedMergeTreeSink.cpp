...
void ReplicatedMergeTreeSink::consume(Chunk & chunk)
{
    ...
    for (auto & current_block : part_blocks)
    {
        ...
        LOG_DEBUG(
            log,
            "Wrote block with ID '{}', {} rows{}, deduplication blocks: {}, deduplication info: {}",
            block_id,
            current_block.block.rows(), quorumLogMessage(replicas_num),
            fmt::join(current_block.deduplication_info->getBlockIds(current_block.partition_id, deduplicate), ", "),
            current_block.deduplication_info->debug());
    ...
    for (auto & partition : delayed_chunk->partitions)
    {
        ...
        while (true)
        {
...

            {
                ...
                LOG_DEBUG(
                    log,
                    "After filtering by collision, removed rows {}/{} removed tokets {}/{} from origin block, after retry remaining rows: {}, remaining tokens: {}, new deduplication info debug: {}",
                    result.removed_rows,
                    partition.block_with_partition.deduplication_info->getRows(),
                    result.removed_tokens,
                    partition.block_with_partition.deduplication_info->getCount(),
